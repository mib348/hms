<?php 
    ...
    GenerateLog("User accessed WardTypes Admin Page", ucwords($strModule));
?>
<style>
...
<form id="param_form">
	...
	<input type="hidden" id="strSubModule" name="strSubModule" value="WardTypes" /> 
...
<div class="content-heading">WardTypes</div>
<div class="row">
	<div class="col-xs-12">
		<div class=" panel panel-default">
			<div class="panel-body">
				...
				<form id="insertion_form" data-parsley-validate="">
					<input type="hidden" id="nWardTypeId" name="nWardTypeId" value="" />
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
							<input type="text" class="form-control" id="strWardType" name="strWardType" required/>
						</div>
						<div class="col-xs-12 col-sm-2">
...
<script type="text/javascript">
	...
	$(function(){
		...
		$(document).on("click", ".delete_btn", function(){
			$("#nWardTypeId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Delete" + "&" + $("#param_form").serialize() + "&nWardTypeId=" + $(this).parent().parent().attr("id"),
		        success: function(data)
		        { 
		        	getList($("#nPage").val(), $("#strOrderColumn").val(), $("#strSortOrder").val(), false);
		        }
...
// 		      setTimeout(function(){window.scrollTo(0, $("#company_table").offset().top);}, 400);
		...
		$(document).on("click", ".edit_btn", function(){
			$("#nWardTypeId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Get" + "&" + $("#param_form").serialize() + "&nWardTypeId=" + $(this).parent().parent().attr("id"),
		        dataType:"json",
		        cache:false,
		        success: function(data)
		        {
		        	$("#strWardType").val(data['rstRow']['WardTypeName']);
	...
	function getList(nPage, strOrderColumn, strSortOrder, bSort=true){
    	...
    	$.ajax({
            ...
            cache:false,
            async:false,
            data: "strAction=getWardTypes&nPage=" + nPage + "&" + "&strSortColumnIndex=" + strOrderColumn + "&strSortOrder=" + strSortOrder + "&" + $("#filter_form").serialize(),
            dataType:"json",
            success:function(data){
                if(data != null){
