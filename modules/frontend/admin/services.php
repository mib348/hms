<?php 
    ...
    GenerateLog("User accessed Services Admin Page", ucwords($strModule));
?>
<style>
...
<form id="param_form">
	...
	<input type="hidden" id="strSubModule" name="strSubModule" value="Services" />
...
<div class="content-heading">Services</div>
<div class="row">
	<div class="col-xs-12">
		<div class=" panel panel-default">
			<div class="panel-body">
				...
				<form id="filter_form" role="form">
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							...
							<?php TableComboMsSql("SubServices", "SubServiceName", "SubServiceId", " IsActive = 1 order by SubServiceName asc", "nFilterSubServiceId", "", "", "--- Select ---", "", "form-control searchable");?>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
				...
				<form id="insertion_form" data-parsley-validate="">
					<input type="hidden" id="nServiceId" name="nServiceId" value="" />
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							<label>Sub Service</label> 
							<?php TableComboMsSql("SubServices", "SubServiceName", "SubServiceId", " IsActive = 1 order by SubServiceName asc", "nSubServiceId", "", "", "--- Select ---", "", "form-control searchable", "required");?>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
							<input type="text" class="form-control" id=strService name="strService" required/>
						</div>
						<div class="col-xs-12 col-sm-2">
...
<script>
	...
	$(function(){
		...
		$(document).on("click", ".delete_btn", function(){
			$("#nServiceId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Delete" + "&" + $("#param_form").serialize() + "&nServiceId=" + $(this).parent().parent().attr("id"),
		        success: function(data)
		        { 
		        	getList($("#nPage").val(), $("#strOrderColumn").val(), $("#strSortOrder").val(), false);
		        }
		...
		$(document).on("click", ".edit_btn", function(){
			$("#nServiceId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Get" + "&" + $("#param_form").serialize() + "&nServiceId=" + $(this).parent().parent().attr("id"),
		        dataType:"json",
		        cache:false,
		        success: function(data)
		        {
		        	$("#strService").val(data['rstRow']['ServiceName']);
		        	$("#nSubServiceId").val(data['rstRow']['SubServiceId']).trigger('change');
	...
	function getList(nPage, strOrderColumn, strSortOrder, bSort=true){
    	...
    	$.ajax({
            ...
            cache:false,
            data: "strAction=getServices&nPage=" + nPage + "&" + "&strSortColumnIndex=" + strOrderColumn + "&strSortOrder=" + strSortOrder + "&" + $("#filter_form").serialize(),
            dataType:"json",
            success:function(data){
                if(data != null){
