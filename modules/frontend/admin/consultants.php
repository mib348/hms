<?php 
    ...
    GenerateLog("User accessed Consultants Admin Page", ucwords($strModule));
?>
<style>
...
<div class="content-heading">Consultants</div>
<div class="row">
	<div class="col-xs-12">
		<div class=" panel panel-default">
			<div class="panel-body">
				...
				<form id="filter_form" role="form">
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							...
							<?php TableComboMsSql("Specialities", "Name", "SpecialityId", " IsActive = 1 order by Name asc", "nFilterSpecialityId", "", "", "--- Select ---", "", "form-control searchable");?>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
				...
				<form id="insertion_form" data-parsley-validate="">
					<input type="hidden" id="nConsultantId" name="nConsultantId" value="" />
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							<label>Speciality</label> 
							<?php TableComboMsSql("Specialities", "Name", "SpecialityId", " IsActive = 1 order by Name asc", "nSpecialityId", "", "", "--- Select ---", "", "form-control searchable", "required");?>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
							<input type="text" class="form-control" id="strConsultant" name="strConsultant" required/>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Email</label> 
						...
						<div class="col-xs-12 col-sm-2">
							<label>Phone</label> 
							<input type="number" class="form-control" id="strPhone" name="strPhone" min="0" />
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Address</label> 
...
<script>
	...
	$(function(){
		...
		$(document).on("click", ".delete_btn", function(){
			$("#nConsultantId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Delete" + "&" + $("#param_form").serialize() + "&nConsultantId=" + $(this).parent().parent().attr("id"),
		        success: function(data)
		        { 
		        	getList($("#nPage").val(), $("#strOrderColumn").val(), $("#strSortOrder").val(), false);
		        }
		...
		$(document).on("click", ".edit_btn", function(){
			$("#nConsultantId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Get" + "&" + $("#param_form").serialize() + "&nConsultantId=" + $(this).parent().parent().attr("id"),
		        dataType:"json",
		        cache:false,
		        success: function(data)
		        {
		        	$("#strConsultant").val(data['rstRow']['Name']);
		        	$("#nSpecialityId").val(data['rstRow']['SpecialityId']).trigger('change');
		        	$("#strEmail").val(data['rstRow']['Email']);
		        	$("#strPhone").val(data['rstRow']['Mobile']);
		        	$("#strAddress").val(data['rstRow']['Address']);
	...
	function getList(nPage, strOrderColumn, strSortOrder, bSort=true){
    	...
    	$.ajax({
            ...
            cache:false,
            data: "strAction=getConsultants&nPage=" + nPage + "&" + "&strSortColumnIndex=" + strOrderColumn + "&strSortOrder=" + strSortOrder + "&" + $("#filter_form").serialize(),
            dataType:"json",
            success:function(data){
                if(data != null){
