<?php 
    ...
    GenerateLog("User accessed Wards Admin Page", ucwords($strModule));
?>
<style>
...
<form id="param_form">
	...
	<input type="hidden" id="strSubModule" name="strSubModule" value="Wards" />
...
<div class="content-heading">Wards</div>
<div class="row">
	<div class="col-xs-12">
		<div class=" panel panel-default">
			<div class="panel-body">
				...
				<form id="filter_form" role="form">
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							...
							<?php TableComboMsSql("WardTypes", "WardTypeName", "WardTypeId", " IsActive = 1 order by WardTypeName asc", "nFilterWardTypeId", "", "", "--- Select ---", "", "form-control searchable");?>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
				...
				<form id="insertion_form" data-parsley-validate="">
					<input type="hidden" id="nWardId" name="nWardId" value="" />
					<div class="form-group">
						<div class="col-xs-12 col-sm-2">
							<label>Ward Type</label> 
							<?php TableComboMsSql("WardTypes", "WardTypeName", "WardTypeId", " IsActive = 1 order by WardTypeName asc", "nWardTypeId", "", "", "--- Select ---", "", "form-control searchable", "required");?>
						</div>
						<div class="col-xs-12 col-sm-2">
							<label>Name</label> 
							<input type="text" class="form-control" id="strWard" name="strWard" required/>
						</div>
						<div class="col-xs-12 col-sm-2">
...
<script>
	...
	$(function(){
		...
		$(document).on("click", ".delete_btn", function(){
			$("#nWardId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Delete" + "&" + $("#param_form").serialize() + "&nWardId=" + $(this).parent().parent().attr("id"),
		        success: function(data)
		        { 
		        	getList($("#nPage").val(), $("#strOrderColumn").val(), $("#strSortOrder").val(), false);
		        }
		...
		$(document).on("click", ".edit_btn", function(){
			$("#nWardId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Get" + "&" + $("#param_form").serialize() + "&nWardId=" + $(this).parent().parent().attr("id"),
		        dataType:"json",
		        cache:false,
		        success: function(data)
		        {
		        	$("#strWard").val(data['rstRow']['WardName']);
		        	$("#nWardTypeId").val(data['rstRow']['WardTypeId']).trigger('change');
	...
	function getList(nPage, strOrderColumn, strSortOrder, bSort=true){
    	...
    	$.ajax({
            ...
            cache:false,
            data: "strAction=getWards&nPage=" + nPage + "&" + "&strSortColumnIndex=" + strOrderColumn + "&strSortOrder=" + strSortOrder + "&" + $("#filter_form").serialize(),
            dataType:"json",
            success:function(data){
                if(data != null){
