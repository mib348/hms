<?php 
    ...
    GenerateLog("User accessed SubDepartments Admin Page", ucwords($strModule));
?>
...
<form id="param_form">
	...
	<input type="hidden" id="strSubModule" name="strSubModule" value="SubDepartments" />
	...
	<!-- Modal -->
	<div class="modal fade text-xs-left" id="sub_department_popup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel13" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header bg-blue-grey white">
			...
			<h4 class="modal-title" id="myModalLabel13">Edit Sub-Department</h4>
		  </div>
		  <div class="modal-body">
		  	<form id="popup_form" novalidate>
		  		<input type="hidden" id="nSubDeptId" name="nSubDeptId" value="" />
		  		<div class="form-group row">
		  			<div class="col-xs-12 controls">
		  				Sub Department
		  				<input type="text" id="strEditDepartment" name="strSubDepartment" value="" class="form-control" required/>
		  			</div>
		  		</div>
		  		<div class="form-group row">
		  			<div class="col-xs-12 controls">
		  				Department
		  				<?php TableComboMsSql("Departments", "Name", "DepartmentId", " IsActive = 1 order by Name asc", "nDeptId", "", "", "--- Select ---", "", "form-control", "required");?>
		  			</div>
		  		</div>
		  		<div class="form-group row">
    ...
    <div class="col-xs-12">
    	<div class="card">
        	<div class="card-header">
        		<h4 class="card-title">Search Sub-Department</h4>
            ...
            <div class="card-body collapse" aria-expanded="false" style="">
            	<div class="card-block">
            		<div class="repeater-default">
                        <div data-repeater-list="sub_departments">
                            <div data-repeater-item="">
                                <form class="form row" id="filter_form">
                                    <div class="form-group mb-1 col-sm-12 col-md-3">
										<label for="strFilterName">Department</label>
										<br>
										<?php TableComboMsSql("Departments", "Name", "DepartmentId", " IsActive = 1 order by Name asc", "nFilterDeptId", "", "", "--- Select ---", "", "form-control");?>
									</div>
                                    <div class="form-group mb-1 col-sm-12 col-md-3">
            ...
            <div class="card-body">
                <div class="card-block">
                		...
                		<div class="col-xs-12  form-group">
            				<div class="controls">
            					<div class="input-group"> 
    								<input type="text" class="form-control" id="strSubDepartment" name="strSubDepartment" placeholder="Add Sub Department" aria-describedby="button-addon2" required data-validation-required-message="This field is required" />
                	...
                	<div class="table-responsive">
                		<table id="sub_department_table" class="table table-bordered table-hover table-condensed table-vcenter">
                			<thead>
                				<tr>
                					<th>ID</th>
                					<th>Name</th>
                			...
                			<tbody id="sub_department_table_tbody">
                			</tbody>
                		</table>
                	</div>
...
<script>
	$(function(){
		...
		$(document).on("click", ".delete_btn", function(){
			$("#nSubDeptId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Delete" + "&" + $("#param_form").serialize() + "&nSubDeptId=" + $(this).parent().parent().attr("id"),
		        success: function(data)
		        { 
		        	getList(1);
		        }
		...
		$(document).on("click", ".edit_btn", function(){
	      ...
	      $.ajax({
	        type: 'POST',
	        url: "../../backend/admin/operations.php",
	        data: "strAction=Get" + "&" + $("#param_form").serialize() + "&nSubDeptId=" + $(this).parent().parent().attr("id"),
	        dataType:"json",
	        cache:false,
	        success: function(data)
	        { 
	        	$("#popup_form #strEditDepartment").val(data['rstRow']['Name']);
	        	$("#popup_form #nDeptId").val(data['rstRow']['DepartmentId']);
        		...
        		$("#sub_department_popup").modal("show");
	        }
	      });
		});
	...
	function getList(nPage, strOrderColumn, strSortOrder){
		...
		$("#sub_department_table_tbody").html('');
		$(".loader-wrapper").show();
    	...
    	$.ajax({
            ...
            cache:false,
            data: "strAction=getSubDepartments&nPage=" + nPage + "&" + "&strSortColumnIndex=" + strOrderColumn + "&strSortOrder=" + strSortOrder + "&" + $("#filter_form").serialize(),
            dataType:"json",
            success:function(data){
                if(data != null){
                    $("#sub_department_table_tbody").html(data);
                    $(".loader-wrapper").hide();
                }
                else{
                	$("#sub_department_table_tbody").html('');
