<?php 
    ...
    GenerateLog("User accessed Departments Admin Page", ucwords($strModule));
?>
...
<form id="param_form">
	...
	<input type="hidden" id="strSubModule" name="strSubModule" value="Departments" />
	...
	<!-- Modal -->
	<div class="modal fade text-xs-left" id="department_popup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel13" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header bg-blue-grey white">
			...
			<h4 class="modal-title" id="myModalLabel13">Edit Department</h4>
		  </div>
		  <div class="modal-body">
		  	<form id="popup_form" novalidate>
		  		<input type="hidden" id="nDeptId" name="nDeptId" value="" />
		  		<div class="form-group row">
		  			<div class="col-xs-12 controls">
		  				<input type="text" id="strEditDepartment" name="strDepartment" value="" class="form-control" required/>
		  			</div>
		  		</div>
		  		<div class="form-group row">
            ...
            <div class="card-body">
                <div class="card-block">
                		...
                		<div class="col-xs-12  form-group">
            				<div class="controls">
            					<div class="input-group"> 
    								<input type="text" class="form-control" id="strDepartment" name="strDepartment" placeholder="Add Department" aria-describedby="button-addon2" required data-validation-required-message="This field is required" />
                	...
                	<div class="table-responsive">
                		<table id="department_table" class="table table-bordered table-hover table-condensed table-vcenter">
                			<thead>
                				<tr>
                					<th>ID</th>
                					<th>Name</th>
...
<script>
	...
	$(function(){
		...
		$(document).on("click", ".delete_btn", function(){
			$("#nDeptId").val($(this).parent().parent().attr("id"));
		      $.ajax({
		        type: 'POST',
		        url: "../../backend/admin/operations.php",
		        data: "strAction=Delete" + "&" + $("#param_form").serialize() + "&nDeptId=" + $(this).parent().parent().attr("id"),
		        success: function(data)
		        { 
		        	getList(1);
		        }
		...
		$(document).on("click", ".edit_btn", function(){
	      ...
	      $.ajax({
	        type: 'POST',
	        url: "../../backend/admin/operations.php",
	        data: "strAction=Get" + "&" + $("#param_form").serialize() + "&nDeptId=" + $(this).parent().parent().attr("id"),
	        dataType:"json",
	        cache:false,
	        success: function(data)
	        { 
	        	$("#strEditDepartment").val(data['rstRow']['Name']);
	...
	function getList(nPage, strOrderColumn, strSortOrder){
    	...
    	$.ajax({
            ...
            cache:false,
            data: "strAction=getDepartments&nPage=" + nPage + "&" + "&strSortColumnIndex=" + strOrderColumn + "&strSortOrder=" + strSortOrder + "&" + $("#filter_form").serialize(),
            dataType:"json",
            success:function(data){
                if(data != null){
                    $("#department_table_tbody").html(data);
                    $(".loader-wrapper").hide();
                }
                else{
                	$("#department_table_tbody").html('');
